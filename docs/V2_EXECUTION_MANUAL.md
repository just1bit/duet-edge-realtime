...
Stage 01 将当前运行路径写入 `outputs/.v2-current`，后续命令默认使用该运行。切换已有运行时，
...
继续条件：终端打印新的绝对 `RUN_ROOT`，且目录中存在 `config.json`。
...
继续条件：`calibration.json.status` 为 `finalized`，并且 `config.sha256` 存在。
...
继续条件：命令输出状态中的 `model.state` 为 `ready`。
...
继续条件：状态中的 `stream.state` 为 `ready`，`session.state` 仍为 `idle`。
...
继续条件：状态中的 `viewer.state` 为 `ready`，浏览器能够打开 Viewer 并连接 WebSocket。
...
```bash
bash scripts/v2_execution/07_input.sh --input /absolute/path/input.pkl
```
...
输入需要带 `motion_151` 的 `.npz`。十分钟发布验收使用
`data+checkpoint/stitched_long_input/stitched_long_input.pkl`。

输入清单记录 `run_id`、配置哈希、输入哈希和锁定时间。证据副本写入
`evidence/input.json`。

继续条件：`input-manifest.json.status` 为 `locked`，`passed` 为 `true`，路径、时长和哈希
与计划输入一致。
...
继续条件：终端打印 `Formal run completed`，且 `summary.json` 和 `stream.ndjson` 均存在。
...
结果写入 `gate-results.json` 和 `report.md`。十分钟 CUDA 发布验收使用：

```bash
bash scripts/v2_execution/09_verify_report.sh --release
```

继续条件：`gate-results.json.passed` 为 `true`，报告中的适用验收项全部通过。
...
| 后端 | `backend` | 输入 | 用途 |
|---|---|---|---|
| CUDA | `cuda` | AIST `.pkl`，包含 `pos` 和 `q` | 模型质量、性能和发布验收 |
