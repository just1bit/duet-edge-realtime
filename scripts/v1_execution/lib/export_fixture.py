#!/usr/bin/env python3
"""Export one real model window as a reusable acceptance fixture."""
...
def main() -> None:
    ...
    try:
        ...
        generated = backend.infer(MotionWindow(0, 0, 150, 5.0, 42, lead))
        unnormalized = backend.unnormalize(generated.motion[None])[0]
        generated_processor = OnlineContinuityProcessor(backend)
        generated_joints = np.concatenate([generated_processor.process(generated.motion), generated_processor.flush()])
        lead_processor = OnlineContinuityProcessor(backend)
        lead_joints = np.concatenate([lead_processor.process(lead), lead_processor.flush()])
